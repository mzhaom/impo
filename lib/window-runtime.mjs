...
export const tmuxFormats = {
  ...
  // so windowFromTmuxRow takes everything from its index onward. Any new field
  // goes BEFORE it — @tm_pinned is a fixed "1"/empty flag, so it is safe there.
  windows:
    "#{window_id}\t#{window_index}\t#{window_name}\t#{window_active}\t#{window_panes}\t#{window_flags}\t#{pane_current_command}\t#{pane_tty}\t#{pane_current_path}\t#{@tm_pinned}\t#{@tm_annotation}",
  panes:
...
export function windowFromTmuxRow(fields) {
  requireFieldCount(fields, 11, "window");
  const [id, index, name, active, panes, flags, activeCommand, tty, cwd, pinned] = fields;
  requireRuntimeId(id, "window");
  // annotation stays the TAIL (free text, may contain tabs).
  const annotation = fields.slice(10).join("\t");
  return {
    id,
    ...
    tty: tty || "",
    cwd: cwd || "",
    // Pin is an ordering preference stored ON the window so it survives a
    // browser/device change (localStorage did not). tmux user options are
    // strings; "1" = pinned, unset/empty = not.
    pinned: pinned === "1",
...
class TmuxWindowRuntime {
  ...
  // Persist the pin flag as the @tm_pinned window option, mirroring
  // setWindowNote's @tm_annotation. Unset (rather than "0") when clearing, so a
  // window carries the option only while pinned.
  async setWindowPinned({ windowId, pinned } = {}) {
    requireRuntimeId(windowId, "window");
    if (pinned) {
      await this.tmux(["set-option", "-w", "-t", windowId, "@tm_pinned", "1"]);
    } else {
      await this.tmux(["set-option", "-w", "-t", windowId, "-u", "@tm_pinned"]);
    }
    return { windowId, pinned: Boolean(pinned) };
  }
