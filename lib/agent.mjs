...
// How often to re-announce (HELLO) on an already-open socket, so a controller
// instance that never saw our registration picks us up without a restart. The
// frame is small and the hub treats a same-socket HELLO as idempotent, so this
// is cheap; 30s bounds the invisible window after a deploy.
const HELLO_HEARTBEAT_MS = Number(process.env.AGENT_HELLO_HEARTBEAT_MS) || 30_000;
// How long an unanswered HELLO is tolerated before we treat the socket as
// routed-to-nowhere and force a fresh dial. Must exceed HELLO_HEARTBEAT_MS so a
// single slow reply is not enough to cycle the connection.
const HELLO_ACK_TIMEOUT_MS = Number(process.env.AGENT_HELLO_ACK_TIMEOUT_MS) || 90_000;
...
export function runAgent(
  ...
  // Announce this machine to the controller. Safe to repeat on an OPEN socket:
  // the hub only force-disconnects an older connector when the HELLO arrives on
  // a DIFFERENT socket (existing.ws !== ws), so re-sending on our own socket is
  ...
  //
  // WHY THIS EXISTS (and why the liveness watchdog above does NOT cover it):
  // the watchdog catches a socket whose peer went SILENT. But behind a proxy
  // that terminates TLS for the controller (Cloud Run's frontend), a deploy
  // replaces the backend INSTANCE while the client socket stays established and
  // Engine.IO pings keep flowing from the new instance. The agent therefore
  // never sees a disconnect, never fires "connect", and never re-sends HELLO —
  // while the new instance's in-memory machine registry (lib/hub.mjs `machines`)
  // has no entry for us. Result: the agent believes it is registered, the UI
  // shows "no machines", and it stays that way until the process is restarted
  // by hand. (Observed 2026-09-01: every controller deploy silently emptied the
  // machine list; reproduced by forcing an instance swap with the agent idle.)
  //
  // HELLO is idempotent on our own socket, so simply re-announcing on a timer
  // repairs a forgotten registration within one interval, whatever caused it.
  let helloTimer = null;
  let lastHelloAckMs = 0;
  const startHelloHeartbeat = () => {
    if (helloTimer) return;
    helloTimer = setInterval(() => {
      if (stopped || !socket.connected) return;
      // A HELLO that goes UNANSWERED means this socket no longer reaches a live
      // controller, even though the proxy keeps it established and pinging (so
      // the liveness watchdog above stays quiet — it only sees a silent peer).
      // Frames are being written into a connection routed to a replaced
      // instance. Nothing short of a fresh dial recovers: force one.
      if (lastHelloAckMs && Date.now() - lastHelloAckMs > HELLO_ACK_TIMEOUT_MS) {
        logEvent("agent_hello_unacked_reconnect", {
          controller: new URL(hubUrl).origin,
          machine: machineName,
          agentId,
          silentMs: Date.now() - lastHelloAckMs,
          message:
            "HELLO went unanswered past the ack timeout on an open socket; forcing reconnect (controller instance replaced behind the proxy).",
        });
        lastHelloAckMs = Date.now(); // don't re-fire while the cycle completes
        try {
          socket.disconnect();
          socket.connect();
        } catch {}
      ...
      sendHello().catch(() => {}); // transient describeMux failure: try again next tick
    }, HELLO_HEARTBEAT_MS);
    helloTimer.unref?.();
  };
  const stopHelloHeartbeat = () => {
    if (helloTimer) {
